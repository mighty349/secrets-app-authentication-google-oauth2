...
app.post("/register", async (req, res) => {
  ...
  try {
    const checkResult = await db.query("SELECT * FROM husers WHERE email = $1", [
      email,
    ]);
      ...
      bcrypt.hash(password, saltRounds, async (err, hash) => {
        if (err) {
          ...
          const result = await db.query(
            "INSERT INTO husers (email, password) VALUES ($1, $2) RETURNING *",
            [email, hash]
          );
...
passport.use(
  ...
  new Strategy(async function verify(username, password, cb) {
    try {
      const result = await db.query("SELECT * FROM husers WHERE email = $1 ", [
        username,
      ]);
      ...
      try {
        ...
        if (result.rows.length === 0) {
          const newUser = await db.query(
            "INSERT INTO husers (email, hash) VALUES ($1, $2)",
